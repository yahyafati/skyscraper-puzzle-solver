...
{
	int col_vals[5];   // Column Data
	int view_count[4]; // Number of buildings seen from eachsides
	int maxes[4];	   // Height of tallest building seen from eachsides
	int items[4];	   // current buildings seen from eachsides
};

/**
 * argv -> a string containing the list of number given by the user.
...
*/
int is_valid(int res[4][4], int given[4][4])
{
	...
	while (i < 4)
	{
		...
		while (j < 4)
		{
			// Gets the building seen from eachside and puts it into items[4]
			set_items(res, checker.items, i, j);
			if (checker.items[0] && is_col_taken(checker.col_vals[checker.items[0]], i))
				return (0);
			checker.col_vals[checker.items[0]] += (1 << i);
			set_values(checker.view_count, checker.maxes, checker.items);
			j++;
		}
		// Checks if it fits the constraints given by the user.
...
*/
int last_two_values(int res[4][4], int given[4][4], const int *values)
{
	int k;
	int m;
	...
	while (k < 24)
	{
		to_int_array(res[2], values[k]);
		m = 0;
		while (m < 24)
		{
			to_int_array(res[3], values[m]);
			if (is_valid(res, given))
				return (1);
			m++;
		}
		k++;
...
int solve_stuff(int res[4][4], int given[4][4])
{
	...
	while (i < 24)
	{
		to_int_array(res[0], values[i]);
		j = 0;
		while (j < 24)
		{
			to_int_array(res[1], values[j]);
			if (last_two_values(res, given, values))
				return (1);
			j++;
		}
		i++;
...
/**
	...
	If the user input is also not valid:
		1. Contains anything but number
